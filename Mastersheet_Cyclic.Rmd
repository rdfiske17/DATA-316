...
It does not, but if we give it better guesses for where to start for parameters, we should be able to improve the trend.

Recall that b is the period * $2\pi$, and we want our period to be $\frac{2\pi}{365}$. If b=$\frac{1}{2\pi*9.24}$, the period is roughly that.
d is such that it is equally distanced from the minimum and the maximum, so let d=$\frac{max(SunriseTime)+min(SunriseTime)}{2}$. a is the distance from d to the max and d to the min, so let a=$max(SunriseTime)-d=d-min(SunriseTime)$. Using desmos with the above calculated values, we can estimate c to be about 4.7, which puts the maximum at t=0.
